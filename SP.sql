...
BEGIN
    ...
    -- 2. TỔNG TIỀN COC
    SELECT ISNULL(pdc.SO_TIEN, 0) AS TIEN_COC
    FROM PHIEU_DAT_COC pdc 
    JOIN LICH l ON pdc.MA_PDC = l.MA_PDC
    JOIN HOP_DONG_THUE hd ON l.MA_PHIEU = hd.MA_PHIEU
...
BEGIN
    ...
    SELECT TOP 1
        ISNULL(pdc.SO_TIEN, 0) AS TIEN_COC
    FROM HOP_DONG_THUE hd
        INNER JOIN LICH l ON hd.MA_PHIEU = l.MA_PHIEU
        LEFT JOIN PHIEU_DAT_COC pdc ON l.MA_PDC = pdc.MA_PDC
    WHERE hd.MA_HOP_DONG = @MA_HOP_DONG
    AND l.MA_PDC IS NOT NULL
    ORDER BY pdc.NGAY DESC;
END
GO
